import { useState, useRef, useEffect, createRef, isValidElement, CSSProperties } from 'react';
import styled from '@emotion/styled';
import { useClickAway } from 'react-use';
import { TextField, TextFieldProps } from '@mui/material';

type IStartAdornment = (hover: boolean, typingMode: boolean, isFilledMode: boolean) => JSX.Element;
...
export const Input = ({ disabled = false, type = 'input', ...props }: InputProps) => {
    ...
    <div className={props.className} style={props.style}>
      {!!props.label && (
        <Label isLighten={typingMode || isFilledMode}>
          {props.label} {props.required ? <span style={{ color: '#dc553b' }}>*</span> : ''}
        </Label>
      )}
      <InputContainer
        ref={containerRef}
...
const InputContainer = styled.div<InputContainerProps>`
  ...
  ${({ isTypingMode, hasError, theme }) =>
    ...
    `
      ...
      background-color: ${theme.palette.grey[900]};
      border: 1px solid ${hasError ? theme.palette.error.main : theme.palette.info.main};
    `}
  ...
  ${({ isFilledMode, disabled, hasError, theme }) =>
    ...
    !disabled &&
    `
      border: 1px solid ${hasError ? theme.palette.error.main : theme.palette.grey[200]};
    `}
