...
import getIconButtonConfig, { ConfigType } from './IconButton/iconButton.config';
...
export const IconButton = forwardRef<HTMLDivElement, IconButtonProps>((props, ref) => {
  ...
  return (
    <Wrapper onClick={!disabled ? onClick : undefined} $style={style}>
      {variant === 'WithText' ? (
        <WithTextIconButton $color={color} $disabled={disabled} $config={iconButtonConfig.withText}>
          {icon}
          <Text>{title}</Text>
        </WithTextIconButton>
      ) : (
