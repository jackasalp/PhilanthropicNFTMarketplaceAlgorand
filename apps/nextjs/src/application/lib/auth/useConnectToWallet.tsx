...
import { generateTransaction } from '$service/api/mutations/generateTransaction';
import { connectToWallet } from '$service/api/mutations/connectToWallet';
import { signAuthTxn } from '$service/auth/signAuthTxn';
import { getWalletAccountAddress } from '$service/auth/getWalletAccountAddress';
...
export const useConnectToWallet = () => {
  ...
  const { mutateAsync: mutateGenerateTransaction } = useMutation(generateTransaction, {
    onError: (error) => {
  ...
  const onConnectToWallet = (storeUser) => async () => {
    ...
    try {
      ...
      const response = await mutateConnectToWallet({ accountAddress, txn, stxn });
      if (response?.token) {
        Cookies.set(
